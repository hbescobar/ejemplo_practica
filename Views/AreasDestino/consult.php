    ...
    <div class="card shadow border-0">
        <div class="card-body">
            ...
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
                ...
                <a href="<?= getUrl('areasDestino', 'areasDestino', 'getInsert'); ?>" class="btn btn-success">
                    <i class='bx bx-plus-circle'></i> Registrar Nueva Área destino
                </a>
            </div>
                    ...
                    <tbody>
                        <?php if (!empty($areas)) : ?>
                            <?php foreach ($areas as $area) : ?>
                                <tr>
                                    <td><?= htmlspecialchars($area['id_area_destino']) ?></td>
                                    <td><?= htmlspecialchars($area['nombre']) ?></td>
                                    <td><?= htmlspecialchars($area['descripcion']) ?></td>
                                    <td>
                                        <div class="d-flex justify-content-center gap-2">
                                            <a href="<?= getUrl('areasDestino', 'areasDestino', 'getEdit', ['id' => $area['id_area_destino']]); ?>" class="btn btn-sm btn-warning" title="Editar">
                                                <i class='bx bx-edit-alt'></i>
                                            </a>
                                            <a href="#" 
                                            class="btn btn-sm btn-danger btn-eliminar-area" 
                                            data-url="<?= getUrl('areasDestino', 'areasDestino', 'delete', ['id' => $area['id_area_destino']]); ?>" 
                                            title="Eliminar">
                        ...
                        <?php else : ?>
                            <tr>
                                <td colspan="4" class="text-center">No hay áreas destino registradas.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
