    ...
    <div class="card shadow border-0">
        <div class="card-body">
            <h4 class="fw-bold text-primary text-center mb-4">
                <i class='bx bx-map'></i> Control de errores de carga masiva
            </h4>
            <!-- ejemplo -->
             <!-- ejemplo -->
            <!-- Filtro y búsqueda -->
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
                <div class="input-group" style="max-width: 220px;">
                    ...
                    <select id="filtroTipo" class="form-select">
                        ...
                        <option value="nombre">Por fecha </option>
                        <option value="descripcion">Por Descripción</option>
                        <option value="estado">Por nombre</option>
                    </select>
                </div>
                    ...
                    <tbody>
                        <?php if (!empty($cargas)) : ?>
                            <?php foreach ($cargas as $carga) : ?>
                                <tr>
                                    <td><?= htmlspecialchars($carga['carga_id']) ?></td>
                                    <td><?= htmlspecialchars($carga['carga_fecha_inicio']) ?></td>
                                    <td><?= htmlspecialchars($carga['carga_descripcion']) ?></td>
                                    <td><?= htmlspecialchars($carga['carga_nombre_archivo']) ?></td>
                                    <td>
                                        <div class="d-flex justify-content-center gap-2">
                                            <a href="<?= getUrl('Carga', 'Carga', 'detalle', ['id' => $carga['carga_id']]); ?>" class="btn btn-sm btn-edit" title="Ver detalle">
